...
public class TransactionSearchByAbandonedTest extends Resource4Test {
    ...
    public void setUp() throws Exception {
        ...
        transactionSearch = new TransactionSearchBuilder()
                .withDateRange(new DateRangeBuilder().between(dateFormat.parse("2016/11/09 00:00:00"), dateFormat.parse("2016/11/09 23:59:59")))
                .withReference("reference")
                .withPage(1)
                .withMaxResults(5)
                .addParameter(new ParameterBuilder().withName("param1").withValue("value1"))
                .build();
    ...
    public void shouldSearch() throws Exception {
        ...
        when(httpClient.execute(any(HttpMethod.class), anyString(), anyMap(), any(HttpRequestBody.class))).thenReturn(response);

        DataList<? extends TransactionSummary> data = transactionSearchByAbandoned.execute(pagSeguro, httpClient);

        assertEquals(new Integer(1), data.getTotalPages());
        assertEquals(false, data.isEmpty());

        Iterator<TransactionSummary> authorizationSummaryIterator = (Iterator<TransactionSummary>) data.getData().iterator();
        DateFormat dateFormat = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss");
        ...
        assertEquals(TransactionStatus.APPROVED, preApproval.getStatus());
        assertEquals(TransactionPaymentMethod.Type.CREDIT_CARD, preApproval.getPaymentMethod().getType());
        assertEquals(new BigDecimal(9.99).setScale(2, RoundingMode.HALF_EVEN), preApproval.getGrossAmount());
        assertEquals(new BigDecimal(0.00).setScale(2, RoundingMode.HALF_EVEN), preApproval.getDiscountAmount());
        assertEquals(new BigDecimal(3.33).setScale(2, RoundingMode.HALF_EVEN), preApproval.getFeeAmount());
        assertEquals(new BigDecimal(2.22).setScale(2, RoundingMode.HALF_EVEN), preApproval.getNetAmount());
        assertEquals(new BigDecimal(1.11).setScale(2, RoundingMode.HALF_EVEN), preApproval.getExtraAmount());
    ...
    public void shouldThrowsErrorLib() throws Exception {
        when(httpClient.execute(any(HttpMethod.class), anyString(), anyMap(), any(HttpRequestBody.class))).thenThrow(new IOException());
        transactionSearchByAbandoned.execute(pagSeguro, httpClient);
    }
}